# hey, I'm Shannon 👋

shannondussoye.github.io

---

<!-- STATS:START -->
> 🔄 Last updated: 2026-06-04 06:50 UTC

## 📊 code stats

| metric | value |
|--------|-------|
| 📝 total lines of code | **5,203** |
| 🗂️ repos counted | **10** |
| 💬 commits (last 12 months) | **160** |

## 🏆 lines of code by repo

| repo | distribution | lines |
|------|-------------|-------|
| [gnafer](https://github.com/shannondussoye/gnafer) | ███████████████ | 1,566 |
| [ozbargainer](https://github.com/shannondussoye/ozbargainer) | ███████████████ | 1,549 |
| [nsw-commute-planner](https://github.com/shannondussoye/nsw-commute-planner) | ████████░░░░░░░ | 886 |
| [Airbnb-Capstone-Project](https://github.com/shannondussoye/Airbnb-Capstone-Project) | ██████░░░░░░░░░ | 583 |
| [Springboard](https://github.com/shannondussoye/Springboard) | ███░░░░░░░░░░░░ | 317 |
| [domain-au](https://github.com/shannondussoye/domain-au) | █░░░░░░░░░░░░░░ | 144 |

## 🌐 language breakdown

| language | distribution | share |
|----------|-------------|-------|
| Python | ██████████░░░░░ | 70% |
| R | ██░░░░░░░░░░░░░ | 14% |
| Shell | █░░░░░░░░░░░░░░ | 5% |
| HTML | █░░░░░░░░░░░░░░ | 5% |
| YAML | ░░░░░░░░░░░░░░░ | 3% |

## ✨ fun facts

- 📖 if printed, my code would fill **21 pages** (or ~0.1 novels)
- ☕ estimated **80 coffees** consumed (1 per 2 commits)
- 🚀 **160** commits pushed in the last year
<!-- STATS:END -->
