# hey, I'm Shannon 👋

shannondussoye.github.io

---

<!-- STATS:START -->
> 🔄 Last updated: 2026-06-28 06:26 UTC

## 📊 code stats

| metric | value |
|--------|-------|
| 📝 total lines of code | **5,589** |
| 🗂️ repos counted | **9** |
| 💬 commits (last 12 months) | **194** |

## 🏆 lines of code by repo

| repo | distribution | lines |
|------|-------------|-------|
| [ozbargainer](https://github.com/shannondussoye/ozbargainer) | ███████████████ | 2,648 |
| [gnafer](https://github.com/shannondussoye/gnafer) | ██████████░░░░░ | 1,739 |
| [Airbnb-Capstone-Project](https://github.com/shannondussoye/Airbnb-Capstone-Project) | ███░░░░░░░░░░░░ | 583 |
| [Springboard](https://github.com/shannondussoye/Springboard) | ██░░░░░░░░░░░░░ | 317 |
| [domain-au](https://github.com/shannondussoye/domain-au) | █░░░░░░░░░░░░░░ | 144 |
| [realestate-au](https://github.com/shannondussoye/realestate-au) | ░░░░░░░░░░░░░░░ | 75 |

## 🌐 language breakdown

| language | distribution | share |
|----------|-------------|-------|
| Python | ███████████░░░░ | 75% |
| R | ██░░░░░░░░░░░░░ | 13% |
| HTML | █░░░░░░░░░░░░░░ | 5% |
| Shell | ░░░░░░░░░░░░░░░ | 2% |
| TOML | ░░░░░░░░░░░░░░░ | 2% |

## ✨ fun facts

- 📖 if printed, my code would fill **23 pages** (or ~0.1 novels)
- ☕ estimated **97 coffees** consumed (1 per 2 commits)
- 🚀 **194** commits pushed in the last year
<!-- STATS:END -->
